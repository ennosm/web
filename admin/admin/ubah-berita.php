...
$idberita = $_GET['id'];
$_SESSION['idberita'] = $idberita;

$query = mysqli_query($koneksi, "SELECT * FROM berita WHERE idberita =
'$idberita'");
...
<head>
    ...
    <!-- GDRIVE CSS -->
     <link type="text/css" href="../assets/select2bs5/select2-bootstrap-5-theme.min.css"
     rel=stylesheet>
                    ...
                    enctype="multipart/form-data">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-control" id="idkategori" name="idkategori[]" multiple="multiple" required>
                                <?php
                                // ambil data kategori yang sudah terpilih untuk berita ini
                                $sql = mysqli_query($koneksi, "SELECT idkategori FROM kategoriberita WHERE idberita='$idberita'");
                                $cekkategori = [];
                                while($rowcekkategori = mysqli_fetch_assoc($sql)) {
                                    $cekkategori[] = $rowcekkategori['idkategori'];
                                }
                                $query = mysqli_query($koneksi, "SELECT idkategori,namakategori FROM kategori");
                                while($row = mysqli_fetch_assoc($query)) {
                                    $selected = in_array($row['idkategori'], $cekkategori) ? 'selected' : '';
                                    ?>
                                    <option value="<?php echo $row['idkategori']; ?>" <?php echo $selected; ?>> <?php echo $row['namakategori']; ?></option>;
                                <?php } ?>
                            </select>
                        </div>
                        <div class="mb-3">
